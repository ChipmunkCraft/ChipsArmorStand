...
import com.deoxservices.chipsarmorstand.network.OpenArmorStandServerPacket;
        ...
        if (event.getKeyMapping() == mc.options.keyUse && modifier.isActive() && !event.isCanceled()) {
            ...
            if (Minecraft.getInstance().hitResult instanceof EntityHitResult entityHitResult) {
                ...
                if (target instanceof ArmorStand armorStand) {
                    UUID owner = armorStand.getData(ChipsArmorStand.ARMOR_STAND_DATA.get()).getOwner();
                    boolean locked = armorStand.getData(ChipsArmorStand.ARMOR_STAND_DATA.get()).isLocked();
